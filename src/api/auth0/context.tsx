...
import { JSX, createContext, createSignal, splitProps, useContext } from 'solid-js';
import { getRequestEvent, isServer } from 'solid-js/web';
import { setCookie } from 'vinxi/server';
import { getSession } from '../server';
import refresh from './refresh';
import { Auth0ContextState, Auth0Props, Organization, UserInfo, UserSessionData } from './types';


export const Auth0Context = createContext<Auth0ContextState>();
...
export function Auth0(props: Auth0Props): JSX.Element {
  ...
  const logoutUrl: string = auth0config.logoutUrl || `${import.meta.env.APP_BASE_URL}/auth/logout`
  ...
  const [organization, setOrganization] = createSignal<Organization | undefined>(undefined)
            ...
            try {
              ...
              if (err.name === 'JWTExpired' || err.code === 'ERR_JWT_EXPIRED') {
                ...
                if (refreshToken) {
                  ...
                  await session.update((d: UserSessionData) => (d.accessToken = tokens.access_token));
                  await session.update((d: UserSessionData) => (d.idToken = tokens.id_token));
