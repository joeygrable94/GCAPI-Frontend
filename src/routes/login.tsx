...
export default function Login() {
  ...
  const [loggingIn, { Form }] = createServerAction$(async (form: FormData) => {
    ...
    switch (loginType) {
      case "login": {
        const authAccess = await login({ username, password });
        if (!authAccess) {
          throw new FormError(`Username/Password combination is incorrect`, {
            fields,
          });
        }
        return createUserSession(authAccess, redirectTo);
      }
      case "register": {
        ...
        const authAccess = await login({ username, password });
        if (!authAccess) {
          throw new FormError(`Something went wrong trying to log into the new user account.`, {
            fields,
          });
        }
        return createUserSession(authAccess, redirectTo);
