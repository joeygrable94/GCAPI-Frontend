...
export default function ButtonToggle(props: ButtonToggleProps) {
  ...
  return (
    <ToggleButton
      class={`button toggle-button inline-flex ${width} appearance-none items-center justify-center outline-none transition focus-visible:outline focus-visible:outline-2 ui-disabled:cursor-not-allowed ${buttonSizeKey[size]} ${buttonEdgeCurve[curve]} ${buttonColorKey[color][styleType]}`}
    ...
    >
      {(state) => (
        <Show when={state.pressed()} fallback={props.iconInactive}>
          {props.iconActive}
        </Show>
      )}
    </ToggleButton>
